            ...
            for line in result.stdout.strip().split('\n'):
                if line.startswith('yes:'):
                    ...
                    status = {
                        'connected': True,
                        'ssid': parts[1],
                        'ip': ips.get(device) or ips.get('wlan0')
                    }
                    break
...
def handle_disk_cleanup(data=None):
    ...
    try:
        emit('disk_cleanup_progress', {'message': 'Disk temizliği başlatılıyor (sistem ve sensör logları temizleniyor)...'})
        logger.info("🧹 Starting manual disk cleanup via WebSocket...")

        cleanup_result = perform_disk_cleanup(
            sensor_logger=getattr(kuvoz_server, 'sensor_logger', None),
...
def handle_system_update(data=None):
    """Sistem güncellemesini başlat (git fetch + ff-only merge)."""
    try:
...
def handle_system_reset(data=None):
    """Sistem güncellemesini geri al (git reset --hard)."""
    try:
